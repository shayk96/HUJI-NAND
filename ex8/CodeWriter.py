...
CALC_END_FRAME_AND_RET_ADDER = "@LCL //C_RETURN\nD=M\n@endFrame\nM=D\n@5\nA=D-A\nD=M\n@retAdder\n" \
...
def write_push_from_dicks(comment: str, param1: str, register: str, param2: str) -> str:
...
def write_pop_from_dicks(comment: str, param1: str, register: str, param2: str) -> str:
...
class CodeWriter:
    ...
    comp_counter = 0
    function_counter = 0  # per function

    call_counter = 0  # per call
    func_name = NULL
    ...
    def write_return(self) -> None:
        self.__output_stream.write(CALC_END_FRAME_AND_RET_ADDER)
    ...
    def write_line(self, infile: Parser) -> None:
        ...
        if command == "C_LABEL":
            arg1 = CodeWriter.func_name + "$" + arg1
            self.write_label(arg1, "")
        if command == "C_IF":
            arg1 = CodeWriter.func_name + "$" + arg1
            self.write_if(arg1, "")
        if command == "C_FUNCTION":
            CodeWriter.func_name = arg1
            self.write_function(arg1, str(infile.arg2()), str(CodeWriter.function_counter))
    ...
    def write_push_pop(self, command: str, segment: str, index: int) -> None:
        ...
        if command == "C_PUSH":
            ...
            if segment in CodeWriter.SEGMENTS_BASES:
                line_to_write = write_push_from_dicks(comment, CodeWriter.SEGMENTS_BASES[segment], "M", str(index))
            if segment in CodeWriter.SEGMENTS_ADDRESS:
                line_to_write = write_push_from_dicks(comment, str(index), "A", CodeWriter.SEGMENTS_ADDRESS[segment])
        if command == "C_POP":
            if segment == "static":
                line_to_write = write_static_pop(comment, self.__filename, index)
            if segment in CodeWriter.SEGMENTS_BASES:
                line_to_write = write_pop_from_dicks(comment, CodeWriter.SEGMENTS_BASES[segment], "M", str(index))
            if segment in CodeWriter.SEGMENTS_ADDRESS:
                line_to_write = write_pop_from_dicks(comment, str(index), "A", CodeWriter.SEGMENTS_ADDRESS[segment])
