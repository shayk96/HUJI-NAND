...
class JackTokenizer:
    ...
    def advance(self, jump: int = 1) -> None:
        """Gets the next token from the input and makes it the current token.
        This method should be called if has_more_tokens() is true.
        Initially there is no current token.
        """
        self.__cur_token_index += jump
        self.__cur_token = self.__input_file[self.__cur_token_index]

    def next_token(self, jump: int = 1) -> str:
        """advances the current token to the next token """
        next_index = self.__cur_token_index + jump
        next_token = self.__input_file[next_index]
        return next_token

    def erase_token(self,index):
        del self.__input_file[self.__cur_token_index + index]

    def get_class_name(self) -> str:
        return self.__input_file[1]
